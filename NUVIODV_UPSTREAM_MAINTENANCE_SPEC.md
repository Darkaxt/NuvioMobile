...
- M6: After a verified sync commit is pushed to `cmp-rewrite`, dispatch the existing authenticated NuvioDV prerelease workflow for that exact branch state.
...
| --- | --- | --- | --- |
| Daily verified upstream sync and release | ACTIVE | M1-M6 | Red/green version test; workflow parse; manual real-upstream dispatch; resulting release inspection |
| Three-day Codex maintenance and issue triage | NOT STARTED | M7-M9 | Active heartbeat inspection; prompt reconciliation |
| Final reconciliation | NOT STARTED | M1-M9 | MC1-MC5 checked; blockers and tracked deferrals equal zero |
